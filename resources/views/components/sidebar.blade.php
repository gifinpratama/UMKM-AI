{{--
    Tujuan     : Sidebar navigasi — Neubrutalism style
    Caller     : layouts/app.blade.php
    Dependensi : auth(), route()
--}}
<aside id="sidebar" class="sidebar">
    ...
    <nav style="flex:1;overflow-y:auto;padding:8px 0 80px;">
        @if(auth()->user()->isAdmin())
            ...
            <a href="{{ route('admin.sellers.index') }}" class="sidebar-link {{ request()->routeIs('admin.sellers.index', 'admin.sellers.show') ? 'active' : '' }}">
                <svg width="16" height="16" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
                Kelola Seller
            </a>
            <a href="{{ route('admin.sellers.create') }}" class="sidebar-link {{ request()->routeIs('admin.sellers.create') ? 'active' : '' }}">
        ...
        @else
            ...
            <a href="{{ route('seller.transactions.index') }}" class="sidebar-link {{ request()->routeIs('seller.transactions.*') ? 'active' : '' }}">
            ...
            <a href="{{ route('seller.customers.index') }}" class="sidebar-link {{ request()->routeIs('seller.customers.*') ? 'active' : '' }}">
            ...
            <a href="{{ route('seller.analytics') }}" class="sidebar-link {{ request()->routeIs('seller.analytics*') ? 'active' : '' }}">
    ...
    <div style="position:absolute;bottom:0;left:0;right:0;padding:14px 16px;border-top:3px solid #000;background:var(--color-golden-light);">
        <div style="display:flex;align-items:center;gap:10px;">
            ...
            <div style="flex:1;min-width:0;">
                <p style="font-family:var(--font-display);font-size:13px;font-weight:700;color:#000;margin:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">{{ auth()->user()->name }}</p>
                <p style="font-family:var(--font-body);font-size:10px;color:#555;margin:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">{{ auth()->user()->isAdmin() ? 'Administrator' : auth()->user()->business_name }}</p>
            </div>
